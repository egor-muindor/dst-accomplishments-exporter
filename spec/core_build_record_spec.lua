...
describe("build_record", function()
  ...
  it("omits title when no title_of is provided", function()
    local r = core.build_record({ completed_Boss_deerclops = { cycles = 5, irl = 99 } },
                                { klei_id = "KU_a", name = "N" })
    assert.is_nil(r.achievements["Boss/deerclops"].title)
  end)
end)
